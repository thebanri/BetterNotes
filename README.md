...
> BetterNotes is a working project name. The current version is **0.1.11**, a
...
```bash
chmod +x BetterNotes-0.1.11-x86_64.AppImage
./BetterNotes-0.1.11-x86_64.AppImage
```

If FUSE is unavailable, run with extraction enabled:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./BetterNotes-0.1.11-x86_64.AppImage
```
...
flatpak install --user ./BetterNotes-0.1.11-x86_64.flatpak
...
./BetterNotes-0.1.11-x86_64.AppImage install   # launches the AppImage where it is
